		...
		RoutingDeterministic(eid, sdr, contactPlan)
{
	...
	dtn_ = dtn;
	sContactProb_ = sContactProb;
}

RoutingCgrModel350_Probabilistic::~RoutingCgrModel350_Probabilistic()
{
}
...
void RoutingCgrModel350_Probabilistic::routeAndQueueBundle(BundlePkt * bundle, double simTime)
{
	...
	if( bundle->getQos() == 0 )
	{
		...
		selectedNeighbor = cgrForward(bundle, simTime, arrivalTime );

		if(selectedNeighbor == NULL || pow(sContactProb_,selectedNeighbor->hopCount) < 0.5)
		{
	...
	else if(bundle->getQos() == 1)
	{
		ProximateNode * selectedNeighbor;
		// Call cgrForward from ion (route and forwarding)
		selectedNeighbor = cgrForward(bundle, simTime, hopCount);
		...
		{
			cout<<"RoutingCGRModel350_Probabilistic::routeAndQueueBundle(BundlePkt * bundle, double simTime) - Route by arrival time exist but no by hops!!."<<endl;
			exit(0);
		}
	...
	{
		cout<<"RoutingCGRModel350_Probabilistic::routeAndQueueBundle(BundlePkt * bundle, double simTime) - Bundle QoS parameter has invalid value."<<endl;
		exit(0);
	}
...
RoutingCgrModel350_Probabilistic::ProximateNode* RoutingCgrModel350_Probabilistic::cgrForward(BundlePkt * bundle, double simTime, Mode mode)
{
...
void RoutingCgrModel350_Probabilistic::identifyProximateNodes(BundlePkt * bundle, double simTime, vector<int> excludedNodes, vector<ProximateNode> * proximateNodes, Mode mode)
{
...
void RoutingCgrModel350_Probabilistic::tryRoute(BundlePkt * bundle, CgrRoute * route, vector<ProximateNode> * proximateNodes, Mode mode)
{
...
void RoutingCgrModel350_Probabilistic::loadRouteList(int terminusNode, double simTime)
{
...
void RoutingCgrModel350_Probabilistic::findNextBestRoute(Contact * rootContact, int terminusNode, CgrRoute * route)
{
	// increment counter
	dijkstraCalls++;
...
void RoutingCgrModel350_Probabilistic::recomputeRouteForContact()
{
	//cout << "***RecomputeRouteForContact not implemented yet!, ignoring route***" << endl;
}

void RoutingCgrModel350_Probabilistic::enqueueToNeighbor(BundlePkt * bundle, ProximateNode * selectedNeighbor)
{
...
void RoutingCgrModel350_Probabilistic::enqueueToLimbo(BundlePkt * bundle)
{
...
void RoutingCgrModel350_Probabilistic::bpEnqueue(BundlePkt * bundle, ProximateNode * selectedNeighbor)
{
...
CgrRoute* RoutingCgrModel350_Probabilistic::getCgrBestRoute(BundlePkt * bundle, double simTime)
{
...
vector<CgrRoute> RoutingCgrModel350_Probabilistic::getCgrRoutes(BundlePkt * bundle, double simTime)
{
	if (!printDebug_) // disable cout if degug disabled
		cout.setstate(std::ios_base::failbit);
...
int RoutingCgrModel350_Probabilistic::getDijkstraCalls()
{
	return dijkstraCalls;
}

int RoutingCgrModel350_Probabilistic::getDijkstraLoops()
{
	return dijkstraLoops;
}

int RoutingCgrModel350_Probabilistic::getRouteTableEntriesExplored()
{
	return tableEntriesExplored;
}

bool RoutingCgrModel350_Probabilistic::msgToMeArrive(BundlePkt * bundle)
{
	/*
...
{
	RoutingCgrModel350_Probabilistic * other = check_and_cast<RoutingCgrModel350_Probabilistic *>(check_and_cast<Dtn *>(
 ...
 */
void RoutingCgrModel350_Probabilistic::successfulBundleForwarded(long bundleId, Contact * contact,  bool sentToDestination)
{
 ...
 */
bool RoutingCgrModel350_Probabilistic::isDeliveredBundle(long bundleId)
{
